...
class Trie:
    ...
    def generating_process(self, distribution_plan):
        for key, value in self._prefix_leaf_nodes.items():
            # We have current node depth. Investigate which level is it.
            ...
            # Check which depth-level has current node
            for depth_level, interval in self._intervals.items():
                if interval[0] <= key < interval[1]:
                    prefix_depth_level = depth_level
                    break

            # get prefixes length from following depth level which could be generated from current level
            generated_info_keys = list(distribution_plan[prefix_depth_level + 1]['generated_info'].keys())
            ...
            values = distribution_plan[prefix_depth_level + 1]['generated_info'][prefix_depth]

            # generate prefix and decrease number of prefixes which should be generated by one
            if values - 1 == 0:
                print(f"Current prefix depth is {key}")
                print(f"Will generate 1 prefix on depth {prefix_depth} and pop this level")
                # calculate new len for future prefix
                distribution_plan[prefix_depth_level + 1]['generated_info'].pop(prefix_depth)
            else:
                print(f"Current prefix depth is {key}")
                print(f"Will generate 1 prefix on depth {prefix_depth} ")
                # calculate new len for future prefix
                distribution_plan[prefix_depth_level + 1]['generated_info'][prefix_depth] = values - 1
